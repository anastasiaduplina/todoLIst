...
class ActionState extends _$ActionState {
  ...
  Future<List<ActionToDo>> build() async {
    List<ActionToDo> list =
        await ref.read(Providers.actionRepositoryProvider).getAll();
    AppLogger.d(
  ...
  Future<void> addOrEditAction(ActionToDo action) async {
    ...
    if (action.id.isEmpty) {
      await ref.read(Providers.actionRepositoryProvider).addAction(action);
      list = await ref.read(Providers.actionRepositoryProvider).getAll();
    } else {
      await ref.read(Providers.actionRepositoryProvider).editAction(action);
      list = await ref.read(Providers.actionRepositoryProvider).getAll();
    }
  ...
  Future<void> deleteAction(ActionToDo action) async {
    if (action.id.isNotEmpty) {
      await ref.read(Providers.actionRepositoryProvider).deleteAction(action);
      List<ActionToDo> list =
          await ref.read(Providers.actionRepositoryProvider).getAll();
